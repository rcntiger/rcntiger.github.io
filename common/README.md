...
rcntiger.github.io/
  common/
    logger.js      ← 콘솔 로그 (운영/개발 환경 자동 구분) [신규]
    supabase.js    ← Supabase CRUD, Storage, keep-alive, UUID
    utils.js       ← 날짜, D-day, localStorage, 이미지 압축, Toast, 에러 핸들러
    telegram.js    ← Telegram Bot 알림 (포맷 템플릿 포함)
    kakao.js       ← Kakao Maps 초기화, 마커, 클러스터, 역지오코딩, 출발지 관리
    kakao-ui.js    ← 지도 툴바 UI (검색·위성·거리재기·로드뷰·출발지·아이콘) [신규]
    icon.js        ← 커스텀 마커 아이콘 관리 [신규]
    excel.js       ← Excel 읽기/쓰기/UI모달/다중필터 (SheetJS 통합, 전면 개편)
```
...
| `handleError(err, options?)` | 에러를 콘솔+Toast+Telegram 동시 처리 [신규] |
...
// 에러를 콘솔 + Toast + Telegram 동시 처리 [신규]
...
기존 `ExcelReader`(UI 전용)와 `ExcelUtil`(로직 전용) 두 모듈로 나뉘어 있던 것을 하나로 통합했습니다.

### SheetJS 로드 / 읽기

| 함수 | 설명 |
|---|---|
| `loadSheetJS()` | SheetJS 동적 로드 (없으면 CDN에서 자동) [신규] |
| `ACCEPT` | 지원 파일 형식 문자열 (xlsx·xls·csv·ods·tsv·html, input accept용) |
| `forwardFill(rows)` | 2D 배열 병합 셀 처리 [신규] |
| `readFile(file, options)` | Excel → 객체 배열 (UI 없이, `file.arrayBuffer()` 방식으로 변경되어 FileReader 불필요) |
...
기존엔 다크 테마가 고정이었으나, CSS 변수로 앱이 색을 직접 제어하도록 변경했습니다.
